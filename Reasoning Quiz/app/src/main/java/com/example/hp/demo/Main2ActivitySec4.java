...
public class Main2ActivitySec4 extends AppCompatActivity {
    List<QuestionAnalogy> quesList1;   // x
    public int score=0;
    int ctr1=1;
    QuestionAnalogy currentQ1;   //x
    ...
    public ArrayList<String> wrongQuestListAnalogy = new ArrayList<String>();   // x 3
    public ArrayList<String> selectedAnsAnalogy = new ArrayList<String>();
    public ArrayList<String> actualAnswerAnalogy = new ArrayList<String>();
    int number;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final Main2ActivitySec4.CounterClass timer = new Main2ActivitySec4.CounterClass(600000, 1000);  //
        timer.start();
        quesList1=db.getAllQuestions4(tableName,catName); //x
        for(int i=0;i<11;i++){
            while(true){
        ...
        butNext1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (currentQ1.getANSWER4().equals(answer.getText())) {  // compare database answer to current answer
                    score++;    // inc score
                }
                else
                {
                    wrongQuestListAnalogy.add(number, currentQ1.getQUESTION4());   // add current wrong question   x 3
                    selectedAnsAnalogy.add(number, answer.getText().toString());   // add current selected answer
                    actualAnswerAnalogy.add(number, currentQ1.getANSWER4());    // add current correct answer
                    number++;
                }
    ...
    public void showResult(){
        Intent intent = new Intent(Main2ActivitySec4.this, ResultActivity.class);
        Bundle b = new Bundle();  // creating the object of bundle class
        b.putInt("scoreAnalogy", score);//Your score       x
        b.putString("section",tableName);//Your table name
        b.putString("category",catName);//Your category name
        intent.putStringArrayListExtra("wrongQuestions", wrongQuestListAnalogy);   // string array list  x 3
        intent.putStringArrayListExtra("selectedAnswer", selectedAnsAnalogy);
        intent.putStringArrayListExtra("actualAnswer", actualAnswerAnalogy);
    ...
    private void setQuestionView(){
        txtQuestion1.setText(currentQ1.getQUESTION4());   // set question
        rda1.setText(currentQ1.getOPTA4());
        rdb1.setText(currentQ1.getOPTB4());             // set options
        rdc1.setText(currentQ1.getOPTC4());
        rdd1.setText(currentQ1.getOPTD4());
        if(ctr1<10)
