...
public class Main2ActivitySec11 extends AppCompatActivity {

    List<QuestionClassification> quesList1;   // x
    public int score=0;
    int ctr1=1;
    QuestionClassification currentQ1;   //x
    ...
    public ArrayList<String> wrongQuestListClassification = new ArrayList<String>();   // x 3
    public ArrayList<String> selectedAnsClassification = new ArrayList<String>();
    public ArrayList<String> actualAnswerClassification = new ArrayList<String>();
    int number;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main2_sec11);
        ...
        final Main2ActivitySec11.CounterClass timer = new Main2ActivitySec11.CounterClass(600000, 1000);  //
        timer.start();
        quesList1=db.getAllQuestions11(tableName,catName); //x
        for(int i=0;i<11;i++){
            while(true){
        ...
        butNext1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (currentQ1.getANSWER11().equals(answer.getText())) {  // compare database answer to current answer
                    score++;    // inc score
                }
                else
                {
                    wrongQuestListClassification.add(number, currentQ1.getQUESTION11());   // add current wrong question   x 3
                    selectedAnsClassification.add(number, answer.getText().toString());   // add current selected answer
                    actualAnswerClassification.add(number, currentQ1.getANSWER11());    // add current correct answer
                    number++;
                }
    ...
    public void showResult(){
        Intent intent = new Intent(Main2ActivitySec11.this, ResultActivity.class);
        Bundle b = new Bundle();  // creating the object of bundle class
        b.putInt("scoreClassification", score);//Your score       x
        b.putString("section",tableName);//Your table name
        b.putString("category",catName);//Your category name
        intent.putStringArrayListExtra("wrongQuestions", wrongQuestListClassification);   // string array list  x 3
        intent.putStringArrayListExtra("selectedAnswer", selectedAnsClassification);
        intent.putStringArrayListExtra("actualAnswer", actualAnswerClassification);
    ...
    private void setQuestionView(){
        txtQuestion1.setText(currentQ1.getQUESTION11());   // set question
        rda1.setText(currentQ1.getOPTA11());
        rdb1.setText(currentQ1.getOPTB11());             // set options
        rdc1.setText(currentQ1.getOPTC11());
        rdd1.setText(currentQ1.getOPTD11());
        if(ctr1<10)
