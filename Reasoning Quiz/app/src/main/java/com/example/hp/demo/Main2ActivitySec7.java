...
public class Main2ActivitySec7 extends AppCompatActivity {

    List<QuestionLogical_Seq> quesList1;   // x
    public int score=0;
    int ctr1=1;
    QuestionLogical_Seq currentQ1;   //x
    ...
    public ArrayList<String> wrongQuestListLogical_Seq = new ArrayList<String>();   // x 3
    public ArrayList<String> selectedAnsLogical_Seq = new ArrayList<String>();
    public ArrayList<String> actualAnswerLogical_Seq = new ArrayList<String>();
    int number;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main2_sec7);
        ...
        final Main2ActivitySec7.CounterClass timer = new Main2ActivitySec7.CounterClass(600000, 1000);  //
        timer.start();
        quesList1=db.getAllQuestions7(tableName,catName); //x
        for(int i=0;i<11;i++){
            while(true){
        ...
        butNext1.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (currentQ1.getANSWER7().equals(answer.getText())) {  // compare database answer to current answer
                    score++;    // inc score
                }
                else
                {
                    wrongQuestListLogical_Seq.add(number, currentQ1.getQUESTION7());   // add current wrong question   x 3
                    selectedAnsLogical_Seq.add(number, answer.getText().toString());   // add current selected answer
                    actualAnswerLogical_Seq.add(number, currentQ1.getANSWER7());    // add current correct answer
                    number++;
                }
    ...
    public void showResult(){
        Intent intent = new Intent(Main2ActivitySec7.this, ResultActivity.class);
        Bundle b = new Bundle();  // creating the object of bundle class
        b.putInt("scoreLogical_Seq", score);//Your score       x
        b.putString("section",tableName);//Your table name
        b.putString("category",catName);//Your category name
        intent.putStringArrayListExtra("wrongQuestions", wrongQuestListLogical_Seq);   // string array list  x 3
        intent.putStringArrayListExtra("selectedAnswer", selectedAnsLogical_Seq);
        intent.putStringArrayListExtra("actualAnswer", actualAnswerLogical_Seq);
    ...
    private void setQuestionView(){
        txtQuestion1.setText(currentQ1.getQUESTION7());   // set question
        rda1.setText(currentQ1.getOPTA7());
        rdb1.setText(currentQ1.getOPTB7());             // set options
        rdc1.setText(currentQ1.getOPTC7());
        rdd1.setText(currentQ1.getOPTD7());
        if(ctr1<10)
